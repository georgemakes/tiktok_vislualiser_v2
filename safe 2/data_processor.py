...
class DataProcessor:
    ...
    def _identify_metric_types(self):
        """Identify which metrics are currency, percentage, ratio, or count based on their names"""

        # Define patterns for different types of metrics
        currency_patterns = [
            'cost', 'spend', 'revenue', 'cpm', 'cpc', 'cpa', 'cpv', 'budget',
            'value', 'conversion_value', 'roas', 'rar'
        ]

        percentage_patterns = [
            'cvr', 'ctr', 'rate', 'percent', '%', 'ratio', 'frequency',
            'engagement', 'reach', 'impression_rate', 'conversion_rate',
            'view_rate', 'completion_rate', 'watch_time'
        ]
        ...
        for metric in self.metrics:
            ...
            if any(pattern in metric_lower for pattern in currency_patterns):
